...
public class ClickGui extends GuiScreen {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        */
        ...
        for (Setting setting : selectedModule.settings) {
            ...
            fontRenderer.drawString(setting instanceof NumberSetting ? listeningForNumericInput ?  "Press arrow keys... > " + setting.value : setting.name + " > " + setting.value : setting instanceof ModeSetting ? setting.name + " > " + ((ModeSetting) setting).getMode() : setting.name, (int) posX2 + 5, (int) (posY2 + 40 + offY), -1);
            ...
            if (isInside(mouseX, mouseY, (int) (posX2), (int) (posY2 + 35 + offY), (int) (posX2 + 150), (int) (posY2 + 35 + 17.5 + offY))) {
                if (mouseButton == 0) {
                    if (setting instanceof BooleanSetting) {
                        ...
                        } else if (listeningForNumericInput) {
                            listeningForNumericInput = false;
                        }
                    } else {
                        Main.sendMessage("?!");
            ...
            for (Setting setting : selectedModule.settings) {
                ...
                if (setting instanceof NumberSetting) {
                    ...
                    if (keyCode == Keyboard.KEY_LEFT) {
                        sv -= 1;
                        ((NumberSetting) setting).setValue(sv);
    ...
    public boolean isInside3(int mouseX, int mouseY, double x, double y, double x2, double y2) {
        return (mouseX > x && mouseX < x2) && (mouseY > y && mouseY < y2);
    }
