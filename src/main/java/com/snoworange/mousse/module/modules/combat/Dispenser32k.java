...
public class Dispenser32k extends Module {
    ...
    public void init() {
        ...
        addSetting(autoClose, redstoneDelay, allowVertical, renderCircle);
    }
        ...
        if (this.toggled) {
            ...
            if (enchantedSwordIndex == -1 && !hasPlacedStuff) {
                ...
                if (closestBlockPos != null) {
                    ...
                    if (closestBlockPos != null) {
                        this.placeStuffVertical(dispenserIndex, closestBlockPos, EnumFacing.UP, new Vec3d(closestBlockPos.getX(), closestBlockPos.getY(), closestBlockPos.getZ()));
                    } else {
                        for (BlockPos blockPos : BlockPos.getAllInBox(new BlockPos(mc.player.posX - 3, mc.player.posY - 1, mc.player.posZ - 3), new BlockPos(mc.player.posX + 3, mc.player.posY, mc.player.posZ + 3))) {
            ...
            if (enchantedSwordIndex == -1 && mc.player.openContainer != null && mc.player.openContainer instanceof ContainerHopper && mc.player.openContainer.inventorySlots != null && !mc.player.openContainer.inventorySlots.isEmpty()) {
                ...
                if (clickedHopper && placedHopper && autoClose.enable) {
            ...
            if (prepareToPlaceRedstone) {
                ...
                if (rstick >= redstoneDelay.value && redstonePos != null) {
    ...
    public void placeStuffVertical(int dispenserIndex, BlockPos blockPos, EnumFacing enumFacing, Vec3d vec3d) {

        mc.player.connection.sendPacket(new CPacketEntityAction(mc.player, CPacketEntityAction.Action.START_SNEAKING));

        //Place dispenser
        //if (mc.world.getBlockState(blockPos) instanceof BlockAir) {
            ...
            mc.player.connection.sendPacket(new CPacketPlayer.Rotation(0, pitch, mc.player.onGround));
            placeBlock(blockPos);
            mc.player.swingArm(EnumHand.MAIN_HAND);
