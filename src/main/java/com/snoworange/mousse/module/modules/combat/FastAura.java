            ...
            if (superWeaponIndex != -1) {
                for (EntityPlayer target : mc.world.playerEntities) {
                    if (!target.getName().equals(mc.getSession().getUsername()) && mc.player.getDistance(target) <= 7 && target.getHealth() > 0 && !target.isDead) {
                        ...
                        //Silent
                        if (silent.enable && !(crItemStack.getItem().equals(Items.DIAMOND_SWORD) && EnchantmentHelper.getEnchantmentLevel(Enchantments.SHARPNESS, crItemStack) >= Short.MAX_VALUE)) {
